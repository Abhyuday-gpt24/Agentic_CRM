...
export default async function EditDealPage({
  ...
  const { id, returnTo } = await params;
  ...
  // 🚨 4. Determine Assignable Users (WITH EMAIL FALLBACK)
  ...
  if (authUser.role === "ADMIN") {
    ...
    assignableUsers = allUsers.map((u) => ({
      id: u.id,
      name: u.name || u.email || "Unknown User", // Fallback applied
    }));
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto w-full animate-in fade-in duration-500">
      ...
      {/* 🚨 LAYOUT FIX: Stacked vertically using flex-col */}
      <div className="flex flex-col gap-8">
        {/* TOP COMPONENT: Deal Metadata Form */}
        <div className="bg-[#242E3D] rounded-2xl shadow-lg border border-slate-700/50 p-6 md:p-8">
          ...
          <form action={updateAction} className="space-y-4">
            ...
            {/* Restored your exact Calculated Value styling */}
            ...
            <div className="pt-4 flex justify-end gap-3">
              <Link
                href={targetRoute}
                className="px-6 py-2.5 rounded-lg text-sm font-medium text-slate-300 hover:text-white hover:bg-slate-700 transition"
              >
                Cancel
              </Link>
              <button
                type="submit"
                className="bg-blue-600 text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-700 transition shadow-lg active:scale-95"
              >
                Save Details
              </button>
            </div>
        ...
        {/* BOTTOM COMPONENT: Deal Items Manager */}
        <div className="bg-[#242E3D] rounded-2xl shadow-lg border border-slate-700/50 p-6 md:p-8">
